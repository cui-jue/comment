...
function waitingClose(newPage) {
    return new Promise(resolve => {
        let doing = setInterval(function () {
            if (newPage && newPage.closed || GM_getValue('tabIsClosed')) {
                GM_setValue('tabIsClosed', false);
...
async function start() {
    ...
    if (loggedBox && loggedBox.innerText) {
        ...
        if (settings.ShowMenu) {
            showMenu()
            let loginButton = document.getElementsByClassName("logged-link")[0];
            loginButton.click();
        }
    } else {
        //提醒登录
